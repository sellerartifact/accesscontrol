...
export class AccessControl {
  ...
  lock(): AccessControl {
    ...
    if (!locked) {
      locked = Boolean(utils.deepFreeze(this._grants));
    }

    /* istanbul ignore next */
    if (!locked) {
      throw new AccessControlError(
        `Could not lock grants: ${typeof this._grants}`,
      );
    }
  ...
  removeRoles(roles: string | string[]): AccessControl {
    ...
    utils.eachRole(this._grants, (roleItem: RoleItem) => {
