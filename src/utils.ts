...
export class utils {
   ...
   *  @returns {Boolean}
   */
  static hasDefined(o: { [key: string]: any }, propName: string): boolean {
  ...
  static eachKey(
    object: any,
    callback: (value: string, index: number, array: string[]) => boolean | void,
    thisArg = null,
  ) {
  ...
  static eachRole(
    grants: Grants,
    callback: (role: RoleItem, roleName: string) => void,
  ) {
   ...
   *
   */
  static eachRoleResource(
    grants: Grants,
    callback: (role: string, resource: string) => void,
  ) {
   ...
   *  is invalid.
   */
  static validName(name: string, throwOnInvalid = true): boolean {
   ...
   *  is invalid.
   */
  static hasValidNames(list: any, throwOnInvalid = true): boolean {
    let allValid = true;
  ...
  static getInspectedGrants(o: Grants | IAccessInfo[]): Grants {
    ...
    if (type === 'object') {
      ...
      (o as IAccessInfo[]).forEach((item: IAccessInfo) =>
        utils.commitToGrants(grants, item, true),
      );
    } else {
   ...
   */
  static normalizeActionPossession(
    info: IQueryInfo,
    asString?: false,
  ): IQueryInfo;
  /**
  ...
  static normalizeActionPossession(
    ...
    if (info.possession || s[1]) {
      switch ((info.possession || s[1]).trim().toLowerCase()) {
        ...
        default:
          throw new AccessControlError(
            `Invalid action possession: ${info.possession || s[1]}`,
          );
      }
    } else {
   ...
   *  @throws {AccessControlError} - If invalid role/resource found.
   */
  static normalizeAccessInfo(access: IAccessInfo, all = false): IAccessInfo {
  ...
  static getUnionAttrsOfRoles(grants: Grants, query: IQueryInfo): string[] {
    ...
    roles.forEach((roleName: string, index: number) => {
      ...
      if (resource) {
        ...
        const possessionKey = `${query.action}:${query.possession}`;
        const actionKey = `${query.action}:any`;

        const strArr = (
          (resource as any)[possessionKey] ||
          (resource as any)[actionKey] ||
          []
        ).concat();
        attrsList.push(strArr);
